...
export function ScoreBreakdown({ v }: { v: InstitutionalVerdictV2 }) {
  ...
  const weights: { key: keyof InstitutionalVerdictV2["components"]; label: string; w: number; anchor?: string }[] = [
    ...
    { key: "fundingBias",       label: "معدل التمويل (Futures)", w: rw.funding },
    { key: "oiTrend",           label: "اتجاه العقود المفتوحة (OI)", w: rw.oi },
  ];
      ...
      {open && (
        <div className="px-3 pb-3 space-y-3">
          <div className="text-[11px] text-muted-foreground">
            الدرجة ناتجة عن مجموع مرجَّح لـ 6 مكوّنات أساسية (+2 من المشتقات إن توفّرت)،
            ثم تنعيم EMA، وأخيراً تعديل بصحة السبريد وحالة السوق ({regime}).
            النسب أدناه تُظهر إسهام كل مكوّن في الدرجة.
          </div>
