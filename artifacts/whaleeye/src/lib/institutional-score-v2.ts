...
import type { FuturesSignal } from './futures';
...
export function institutionalScoreV2(
  ...
  price: PriceMetrics,
  klines: Kline[],
  opts: { prevScore?: number; emaAlpha?: number; futures?: FuturesSignal | null } = {}
  ...
  // ── Futures components (optional — only if data available) ──────────────────
  const futures = opts.futures;
  const fundingBias   = futures ? clamp(futures.fundingBias, -1, 1) : 0;
  const oiTrend       = futures ? clamp(futures.oiTrend, -1, 1) : 0;
  const hasFutures    = futures != null && futures.confidence >= 50;
  ...
  const weighted =
    ...
    volumeTrend         * W.vol +
    microDrift          * W.micro +
    (hasFutures ? fundingBias * W.funding : 0) +
    (hasFutures ? oiTrend     * W.oi     : 0);
  ...
  const coreSigns = [bookImbalance, proximityPressure, momentum, microDrift, volumeTrend];
  if (hasFutures) { coreSigns.push(fundingBias); coreSigns.push(oiTrend); }
  const signs = coreSigns.map((c) => (c > 0.08 ? 1 : c < -0.08 ? -1 : 0));
  ...
  const vals = hasFutures
    ? [bookImbalance, proximityPressure, momentum, microDrift, volumeTrend, fundingBias, oiTrend]
  ...
  if (hasFutures) {
    reasoning.push(`معدل التمويل: ${fundingBias > 0 ? "صاعد" : "هابط"} (${(fundingBias * 100).toFixed(1)}%) — ${futures!.reasoning[0]}`);
    reasoning.push(`اتجاه OI: ${oiTrend > 0 ? "تزايد" : "تناقص"} — رأسمال جديد ${oiTrend > 0 ? "يدخل" : "يخرج"}`);
  }
  ...
  return {
    ...
    components: {
      ...
      fundingBias: hasFutures ? fundingBias : 0,
      oiTrend: hasFutures ? oiTrend : 0,
      momentum,
      volumeTrend,
      spreadHealth,
      microDrift,
...
export const REGIME_WEIGHTS: Record<Regime, { book: number; wall: number; mom: number; rsi: number; vol: number; micro: number; funding: number; oi: number }> = {
  // Trending: momentum + volume matter more; RSI less (follow trend)
  trending: { book: 0.20, wall: 0.16, mom: 0.20, rsi: 0.07, vol: 0.18, micro: 0.09, funding: 0.05, oi: 0.05 },
  // Ranging: mean-reversion (RSI + microDrift) matter more; momentum less
  ranging:  { book: 0.25, wall: 0.20, mom: 0.07, rsi: 0.20, vol: 0.09, micro: 0.09, funding: 0.05, oi: 0.05 },
  // Volatile: book + walls matter most; momentum suppressed (whipsaws)
  volatile: { book: 0.27, wall: 0.22, mom: 0.04, rsi: 0.09, vol: 0.18, micro: 0.09, funding: 0.06, oi: 0.05 },
};
