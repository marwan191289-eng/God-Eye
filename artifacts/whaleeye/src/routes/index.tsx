...
import { Radio, Zap, BookOpen, Crosshair, LineChart, FileText, Sliders, FlaskConical, AlertTriangle, GitCompare, Activity, TrendingUp, Database } from "lucide-react";
import { ForecastPanel } from "@/components/trading/ForecastPanel";
import { DataSourcesPanel } from "@/components/trading/DataSourcesPanel";
import { FundingRatePanel } from "@/components/trading/FundingRatePanel";
import { computeForecast } from "@/lib/forecast";
import { computeFuturesSignal } from "@/lib/futures";
import { fetchFundingRate, fetchPremiumIndex, fetchOpenInterest, fetchOIHistory } from "@/lib/binance";
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] text-muted-foreground mono">
              Order-Flow Engine · Binance REST polling (~1s)
            </div>
          </div>
        </div>
    ...
    <footer className="border-t border-border mt-8">
      <div className="max-w-[1600px] mx-auto px-4 md:px-6 py-6 text-center">
        ...
        <div className="text-[11px] text-muted-foreground mt-2">
          البيانات من Binance REST API (استطلاع ~1–15 ثانية، ليس WebSocket) · أداة تحليلية فقط · ليست توصية استثمارية
        </div>
      </div>
    </footer>
  );
...
function SymbolView({
  ...
  // ── Futures data (honest REST polling — real Binance Futures API) ──
  const { data: futuresData } = useQuery({
    queryKey: ["futures", symbol],
    queryFn: async () => {
      const [fundingHistory, premiumIndex, openInterest, oiHistory] = await Promise.all([
        fetchFundingRate(symbol, 8),
        fetchPremiumIndex(symbol),
        fetchOpenInterest(symbol),
        fetchOIHistory(symbol, "1h", 24),
      ]);
      const signal = computeFuturesSignal(fundingHistory, premiumIndex, openInterest, oiHistory, metrics?.mid ?? 0);
      return { fundingHistory, premiumIndex, openInterest, oiHistory, signal };
    },
  ...
  const verdict = useMemo(() => {
    ...
    const v = institutionalScoreV2(metrics, walls, priceMetrics, klines, {
      ...
      emaAlpha: 0.3,
      futures: futuresData?.signal ?? null,
    });
    prevScoreRef.current = v.score;
    return v;
  }, [metrics, walls, priceMetrics, klines, futuresData?.signal]);

  const forecast = useMemo(
    () => computeForecast(verdict as any, cvdStats, ofiStats, smcAnalysis),
    [verdict, cvdStats, ofiStats, smcAnalysis]
  );
    ...
    <div className="space-y-5">
      ...
      {blocked ? (
        ...
        <>
          ...
          {/* Funding Rate + Open Interest — real Binance Futures data */}
          <Panel
            icon={<TrendingUp className="size-4 text-whale" />}
            title="تحليل المشتقات — Funding + Open Interest"
            extra={
              <span className="text-[10px] mono text-muted-foreground">
                {futuresData ? "بيانات حقيقية · Binance Futures" : "جارٍ الجلب..."}
              </span>
            }
          >
            <FundingRatePanel signal={futuresData?.signal ?? null} />
          </Panel>

          {/* Probabilistic Forecast — derived from real signals */}
          <Panel
            icon={<TrendingUp className="size-4 text-primary" />}
            title="التوقع الاحتمالي — مبني على الإشارات الحقيقية المتصلة"
            extra={
              <div className="flex items-center gap-2">
                <span className="text-[10px] mono text-muted-foreground">
                  {forecast.confidence > 0 ? `ثقة: ${forecast.confidence}%` : "بيانات غير كافية"}
                </span>
                <span className={cn(
                  "text-[9px] mono px-1.5 py-0.5 rounded-full border",
                  forecast.regime === "trending" ? "border-bull/30 text-bull bg-bull/5"
                  : forecast.regime === "volatile" ? "border-bear/30 text-bear bg-bear/5"
                  : "border-gold/30 text-gold bg-gold/5"
                )}>
                  {forecast.regime === "trending" ? "اتجاه"
                   : forecast.regime === "volatile" ? "متقلب"
                   : forecast.regime === "ranging" ? "تذبذب"
                   : "غير محدد"}
                </span>
              </div>
            }
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-5">
            ...
            <Panel
              ...
              extra={<span className="text-[10px] mono text-muted-foreground">عمق 100 · REST ~1 ثانية</span>}
            >
         ...
         {/* CVD — Cumulative Volume Delta */}
                    <Panel
                      ...
                      extra={
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] mono text-muted-foreground">تقريبي · دفتر أوامر REST</span>
                        </div>
                      }
                    >
                    ...
                    <Panel
                      ...
                      extra={
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] mono text-muted-foreground">
                            {ofiStats.history.length} لقطة · دلتا ~1ث
                          </span>
                        </div>
                      }
                    >
                    ...
                    {/* RL Agent — local learning simulation (NOT pre-trained, NOT institutional AI) */}
                    <Panel
                      icon={<Database className="size-4 text-muted-foreground" />}
                      title="وكيل التعلم المحلي (REINFORCE) — تجريبي"
                      extra={
                        <span className="text-[9px] mono px-2 py-0.5 rounded-full border border-gold/30 text-gold bg-gold/5">
                          محاكاة محلية · يبدأ من صفر عند كل جلسة
                        </span>
                      }
                    >
                    ...
                    <Panel
                      icon={<Database className="size-4 text-muted-foreground" />}
                      title="مصادر البيانات — شفافية كاملة"
                      extra={<span className="text-[10px] mono text-muted-foreground">ما هو متصل فعلًا وما هو غير متاح</span>}
                    >
                      <DataSourcesPanel />
                    </Panel>
                    ...
                    <div className="mt-6 rounded-2xl border border-border/50 bg-card/20 px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-[11px] mono text-muted-foreground/60 mt-0.5">Order-Flow Engine · Binance REST polling</div>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1.5 text-[10px] text-muted-foreground mono select-none">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span>Binance REST · استطلاع ~1–15 ثانية</span>
                        </div>
                        <div className="flex items-center gap-2 gap-y-1 flex-wrap justify-end">
                          {["CVD", "OFI", "SMC", "Liquidity Zones", "Forecast", "Backtest"].map(tag => (
                        ...
                        <div className="text-[9px] opacity-50 mt-0.5">v3.1 · {new Date().getFullYear()}</div>
                      </div>
                    </div>
